...
6. 修改输入法
   ...
   # （可选）如果切换x11或wayland之后导致输入法缺失了，可以重启 fcitx5
   fcitx5 -r
   ```
...
14. 安装VScode
    ...
    ```
    # sudo snap install code --classic # 已弃用，因为有各种不兼容，尤其是对于输入法
    推荐官网下载 .deb版本，然后运行
...
15. 安装Pycharm（可选）
    ...
    - 官网上下载最新版本的Pycharm的.tar.gz包；

    - 解压到/opt下：

      ```bash
      mkdir -p /opt/jetbrains
      sudo tar -xzvf pycharm-2025.2.4.tar.gz -C /opt/jetbrains
      ```

    - 访问破解网站：https://3.jetbra.in/ 下载破解工具 jetbra 并同上一部安装到/opt下

    - 阅读 jetbra 安装目录下的 readme.txt，并复制关键参数，例如：

      ```bash
      -javaagent:/opt/jetbra/ja-netfilter.jar=jetbrains
      ```

    - 将如上参数写入到安装的 Pycharm 软件的vmoptions中

      ```bash
      /opt/jetbrains/pycharm-2025.2.4/bin
    ...
    - 为最新版本的 Pycharm 添加图标
      ...
      ```bash
      vim /usr/share/applications/jetbrains-pycharm.desktop
      # 添加以下内容，具体路径根据实际修改
      #!/usr/bin/env xdg-open
      [Desktop Entry]
      Name=Pycharm
      Exec=/opt/jetbrains/pycharm-2025.2.4/bin/pycharm %U
      Terminal=false
      Type=Application
      Icon=/opt/jetbrains/pycharm-2025.2.4/bin/pycharm.png
      Comment=pycharm
