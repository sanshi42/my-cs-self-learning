...
5. 安装clash：
   ...
   - 登录SPEEDCAT（可通过outlook进行查找）复制Clash订阅链接：
     ...
     - 安装Clash。
       ...
       方案二：直接使用自己硬盘里面下载好的程序直接复制到原电脑上就行
...
6. 安装 Chrome 浏览器
   ...
   2. 下载后登录自己的 gmail 账号便基本上完全恢复自己的 chrome 能力。
   ```

7. 修改输入法

   ```
   1. 需要将自己的输入法调整为 fcitx 5；
   2. 使用自己下载过的 https://github.com/sanshi42/fcitx5_customizer，clone下来并选择完全自定义。
   ```

8. 安装其他配置文件（需要在安装其他软件之前）
...
10. 安装Flatpak（方便安装其他的ubuntu官方商店不存在的软件，如微信）
...
15. 安装[微信](https://www.yydnas.cn/2023/08/2023.08.16-Ubuntu%E5%AE%89%E8%A3%85%E5%BE%AE%E4%BF%A1%E7%9A%84%E4%B8%A4%E7%A7%8D%E6%96%B9%E6%B3%95/index.html)
